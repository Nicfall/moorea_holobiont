...
#for file in $(cat samples.list); do ~/bin/bbmap/bbduk.sh in1=${file}_R1_NoIll_No4N.fastq in2=${file}_R2_NoIll_No4N.fastq restrictleft=20 k=10 literal=GTGYCAGCMGCCGCGGTA,GGACTACHVGGGTWTCTAAT copyundefined=t outm1=${file}_R1_NoIll_No4N_No16S.fastq outu1=${file}_R1_check.fastq outm2=${file}_R2_NoIll_No4N_No16S.fastq outu2=${file}_R2_check.fastq; done &>bbduk_No16S.log

#moved *_No16S.fastq to a new folder for renaming:

# for longer file names:
# for file in *_No16S.fastq
# do export first6=${file:0:6}
# echo ${first6}
# cp $file ${first6}.fastq
# done
# 
...
path <- "~/Desktop/mrteens/round3/" # CHANGE ME to the directory containing the fastq files after unzipping.
fns <- list.files(path)
fns
...
dim(seqtab.nochim)
#Identified 79 bimeras out of 2355 input sequences.
...
saveRDS(seqtab.nochim, file="~/Desktop/mrteens/mr16s_seqtab.nochim.rds")
saveRDS(taxid, file="~/Desktop/mrteens/mr16s_taxid.rds")

write.csv(seqtab.nochim, file="~/Desktop/mrteens/mr16s_seqtab.nochim.csv")
write.csv(taxid, file="~/Desktop/mrteens/mr16s_taxid.csv")
...
# 236 out of 2276 were assigned to the species level.
# Of which 214 had genera consistent with the input table.

saveRDS(taxa.plus, file="~/Desktop/mrteens/mr16s_taxaplus.rds")
write.csv(taxa.plus, file="~/Desktop/mrteens/mr16s_taxaplus.csv")
...
taxid <- readRDS("mr16s_taxid.rds")
taxaplus <- readRDS("mr16s_taxaplus.rds")
...
plot_bar(ps.top30, x="zone", fill="class") #+ facet_wrap(~ColonyID+Timepoint, scales="free_x")
...
#taxa_sums(ps.mnw)
#ps.mnw.0 <- prune_taxa(taxa_sums(ps.mnw)>0,ps.mnw)
#taxa_sums(ps.mnw.0)
...
path='~/Desktop/mrteens/mr16s.fasta'
...
write.csv(seqtab.nochim,file="mr16s_AllOTUs.csv",quote=F)
##replace sequences with shorter names (correspondence table output below)
taxa_names(ps)<-ids
str(seqtab.nochim)
